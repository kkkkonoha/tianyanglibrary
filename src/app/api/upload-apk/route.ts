...
import { isManagedApkFilename } from "@/lib/apk"

// CI 上传 APK：Bearer token 校验，文件名必须为天央图书馆-vX.Y(.Z).apk
export async function POST(req: NextRequest) {
  ...
  if (!isManagedApkFilename(file.name)) {
    return NextResponse.json({ error: "文件名必须为天央图书馆-vX.Y(.Z).apk" }, { status: 400 })
  }
