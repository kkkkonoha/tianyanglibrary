      ...
      {recs.length === 0 ? (
        ...
        <>
          <div className="space-y-3">
            {recs.map((rec, i) => (
              <Link key={rec.id} href={`/resource/${rec.resource.id}`} className="animate-lib-rise-in" style={{ animationDelay: `${Math.min(i, 12) * 60}ms` }}><Card className="transition-shadow hover:shadow-md"><CardContent className="flex items-center gap-3 p-4">{rec.resource.coverImage ? <img src={rec.resource.coverImage} alt={rec.resource.title} loading="lazy" decoding="async" className="h-12 w-9 shrink-0 rounded object-contain bg-muted/30" /> : null}<div className="min-w-0 flex-1"><h4 className="text-sm font-medium">{rec.resource.title}</h4><Badge variant="secondary" className="mt-1 text-xs"><ResourceTypeLabel type={rec.resource.type} iconClassName="h-3 w-3" /></Badge>{rec.note && <p className="mt-1 line-clamp-1 text-xs text-muted-foreground">&ldquo;{rec.note}&rdquo;</p>}</div></CardContent></Card></Link>
            ))}
          </div>
